...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        # made a list for the filter
        filtered = []
        # go through the calls in data list provided
        for c in data:
            # check if call made or received matches filter_string
            if filter_string in (c.src_number, c.dst_number):
                # if it matches, append to filtered list
                filtered.append(c)
        return filtered
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        # list for calls Less than given time
        LESS = []
        # list for calls Greater than given time
        GREATER = []
        # check if the filter string is empty or less than 2 characters
        if len(filter_string) < 2 or not filter_string:
            return data
        # variable to check if first letter of input is L or G
        LESS_OR_GREATER = filter_string[0]
        # variable for time but convert to int since it is str
        TIME = int(filter_string[1:])
        # check if first letter is G
        if LESS_OR_GREATER == 'G':
            # loop through data and find all calls greater than given time
            for c in data:
                if c.duration > TIME:
                    # append those calls to Greater and return
                    GREATER.append(c)
            return GREATER
        # check if first letter is L
        elif LESS_OR_GREATER == 'L':
            # loop through data and find all calls less than given time
            for c in data:
                if c.duration < TIME:
                    # append those calls to Less and return
                    LESS.append(c)
            return LESS
        # if the first character is not G or L, return data
        else:
            return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        # made list for filter
        filtered = []
        # a list for the 4 coordinates
        lst = []
        for i in len(filter_string.split(',')):
            lst.append(i)
        # check if filter_string has the 4 inputs needed
        if len(lst) != 4:
            return data

        # assign each coordinate to its variable
        LOWER_LONG = float(lst[0])
        LOWER_LAT = float(lst[1])
        UPPER_LONG = float(lst[2])
        UPPER_LAT = float(lst[3])

        # check if the filter_string is within map boundaries
        # took numbers from application.py
        if LOWER_LONG < -79.697878 or LOWER_LAT < 43.576959 \
                or UPPER_LONG > -79.196382 or UPPER_LAT < 43.799568:
            return data

        for c in data:
            # check if this call fits within boundaries
            if (LOWER_LONG <= c.src_loc[0] <= UPPER_LONG
                and LOWER_LAT <= c.src_loc[1] <= UPPER_LAT)\
                    or (LOWER_LONG <= c.dst_loc[0] <= UPPER_LONG
                        and LOWER_LAT <= c.dst_loc[1] <= UPPER_LAT):
                # append to list if within boundaries
                filtered.append(c)
